...
public:
    ...
    quint8 i85read8(const i85stat *, uint16, uint16);
    void i85write8(i85stat *, uint16, uint16, quint8);
    uint16 i85read16(const i85stat *, uint16, uint16);
    void i85write16(i85stat *, uint16, uint16, uint16);
    quint8 i85inp8(const i85stat *, uint16);
    uint16 i85inp16(const i85stat *, uint16);
    void i85out8(i85stat *, uint16, quint8);
    void i85out16(i85stat *, uint16, uint16);
    void i85reset(i85stat *);
    void i85trace(const i85stat *);

    int i85disasm(char *, const i85stat *, uint16, uint16);
    ...
    uint16 getreg16(const i85stat *i85, quint8 reg);
    void setreg16(i85stat *i85, quint8 reg, uint16 x);
    uint16 getsreg(const i85stat *i85, quint8 reg);
    void setsreg(i85stat *i85, quint8 reg, uint16 x);
    void getsegoff(const i85stat *i85, quint8 rm, uint16 *seg, uint16 *off);
    quint8 getrm8(const i85stat *i85, quint8 rm);
    void setrm8(i85stat *i85, quint8 rm, quint8 x);
    uint16 getrm16(const i85stat *i85, quint8 rm);
    void setrm16(i85stat *i85, quint8 rm, uint16 x);
